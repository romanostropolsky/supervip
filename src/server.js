...
async function initSchema() {
  ...
  // Додавання базoвих міст, якщо таблиця порожня
  const res = await pool.query('SELECT COUNT(*) FROM cities');
  if (parseInt(res.rows[0].count) === 0) {
    const defaultCities = ['Київ', 'Львів', 'Одеса', 'Дніпро', 'Харків'];
...
bot.hears('📍 Оновити локацію', async (ctx) => {
  try {
    ...
    // Додавання кнопки GPS на початок
...
// Обробка відправки точних GPS-координат
...
// Обробка вибору міста з текстових кнопок
